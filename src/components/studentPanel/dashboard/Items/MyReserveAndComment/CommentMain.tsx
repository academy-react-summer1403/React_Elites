...
const CommentMain = ({avatar, userName, date, title, description, likeCount, disLikeCount}) => {
    ...
    return (
        <>
            <div className={style.containerComment} data-theme={darkMode ? "darkNoBG" : "lightMode"}>
            <div className={style.header}>
                <img className={style.avatar} src={avatar} />
                <div className={style.column}>
                    <div data-theme={darkMode ? "darkNoBG" : "lightMode"} className={style.userName}>{userName}</div>
                    <div data-theme={darkMode ? "darkNoBGDisc" : "lightMode"} className={style.date}>{dateConvertor(date)}</div>
                </div>
                <div className={style.body}>
                    <div data-theme={darkMode ? "darkNoBG" : "lightMode"} className={style.title}>{title}</div>
                    <div data-theme={darkMode ? "darkNoBG" : "lightMode"} className={style.description}>{description}</div>
                </div>
                <div className={style.footer}>
                    ...
                    <div data-theme={darkMode ? "darkNoBG" : "lightMode"} className={style.count}>{disLikeCount}</div>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
